...
func GetAttributeNameAtOffset2(root *sitter.Node, content string, offset uint) (string, bool) {
	node := HasNodeInHierarchy(root, "attribute_name", offset, offset)
	if node == nil {
		return "", false
	}

	attributeName := node.Utf8Text([]byte(content))
	if attributeName == "" {
		return "", false
	}
